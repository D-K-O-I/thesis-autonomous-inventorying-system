...
class VCM:
	"""
	Virtual Coordinates Map object class.
	Can be executed as a standalone to test the routing algotithm.

	upd_node: updates nodemap
	add_node: adds a node to the nodemap: epc = unique identifier, x = x-axis coordinate, y = y-axis coordinate
	del_node: removes a node from the nodemap: epc = unique identifier
	get_nodemap: returns nodemap
	set_perimeter: ensures path is routed inside the specified perimeter: lower_bound = lower mathematical bound of the coordinates set,
		upper_bound = upper mathematical bound of the coordinates set. Returns nodemap inside perimeter.
	rem_perimeter: removes an existing perimeter



	"""
	def __init__(self, shelf_height, current_pos):
		self.nodemap = {}
		self.nodemap_per = {}
		self.nodemap_hld = {}
		self.shelf_height = shelf_height
		self.current_pos = current_pos #arbitrary, definable
	...
	def add_node(self, epc, x, y):
		if epc in self.nodemap:
			while True:
				user_input = raw_input("EPC already registered to node at " + str(self.nodemap[epc]) + ". Update this node's coordinates?\n [Y]es [N]o\n")
				if user_input == "Y":
	...
	def coordinate_check(self,cpos,x,y):
		...
		#COMMENT THIS FUNCTION TO NOT SKIP OVER 1 HOP "HOLES" IN THE VIRTUAL COORDINATES MAP
		else:
	...
	def route_3d(self):
		...
		print("Shelf levels: 0 -> " + str(self.shelf_height))
	...
	def route(self):
		...
		while len(self.spanning_tree_unreached) > 0:
			...
			for candidate in candidatelist:
				...
				#select shortest path, two answers
					...
					if self.nodemap[candidate][0] == self.nodemap[answer][0] and self.current_pos[0] == self.nodemap[candidate][0]:
						...
						if abs(max - self.current_pos[1]) <= abs(min - self.current_pos[1]):
						...
						elif abs(max - self.current_pos[1]) > abs(min - self.current_pos[1]):
							answer_coord = tuple(map(operator.add, self.current_pos, (0,-int(self.minweight))))
							answerp = [k for k,v in self.nodemap.items() if v == answer_coord]
							#print(answerp)
							try:
					...
					#different rows		
						...
						if abs(max - self.current_pos[0]) < abs(min - self.current_pos[0]):
						...
						elif abs(max - self.current_pos[0]) >= abs(min - self.current_pos[0]):
							answer_coord = tuple(map(operator.add, self.current_pos, (-int(self.minweight),0)))
							answerp = [k for k,v in self.nodemap.items() if v == answer_coord]
							#print(answerp)
							try:
			...
			#if diagonal, move to correct Y value, then to correct X value
			#to be done in movement control!
